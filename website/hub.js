import { escapeHtml, formatGeneratedAt, formatTopArxivBadge } from "./shared.js";
...
const AREA_FILTER_YEARS = [2024, 2025, 2026];
...
function renderTopMonthly(data) {
  ...
  if (heading && period) {
    heading.textContent = `OS Kernel & LLM picks by areas (${period})`;
  }
...
function renderBroadcastCard(pick) {
  ...
  const date = pick.published
    ? `<time class="broadcast-date" datetime="${escapeHtml(pick.published)}">${escapeHtml(formatDate(pick.published))}</time>`
    : "";
...
function renderTodayBroadcast(data) {
  ...
  const dateLabel = data.date_label || "Yesterday";

  section.hidden = false;
  const built = data?.generated_at ? ` | updated ${formatGeneratedAt(data.generated_at)}` : "";
  meta.textContent =
    picks.length > 0
      ? `Posted ${dateLabel} (UTC) | top ${picks.length} papers${built}`
      : `Posted ${dateLabel} (UTC) | no matches${built}`;

  if (!picks.length) {
    list.innerHTML = '<p class="empty empty-compact">No strong papers from yesterday.</p>';
